    ...
    <div class="relative flex flex-col gap-[30px] my-[60px] px-5">
            ...
            <div class="flex flex-col w-full rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white">
                <div class="flex gap-4">
                    <div class="flex w-[120px] h-[132px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                        <img src="{{ url('storage/private/' . $transaction->boardingHouse->thumbnail) }}" class="w-full h-full object-cover" alt="icon">
                    </div>
                ...
                <div class="flex gap-4">
                    <div class="flex w-[120px] h-[138px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                        <img src="{{ url('storage/private/' . ($transaction->room->roomImages->first()->image ?? '')) }}" class="w-full h-full object-cover" alt="icon">
                    </div>
                    <div class="flex flex-col gap-3 w-full">
                        ...
                        <div class="flex items-center gap-[6px]">
                            ...
                            <p class="text-sm text-ngekos-grey">
                                 {{ \Carbon\Carbon::parse($transaction['start_date'])->isoFormat('D MMMM Y') }} - {{ \Carbon\Carbon::parse($transaction['start_date'])->addMonths((int)$transaction['duration'])->isoFormat('D MMMM Y') }}
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="flex flex-col gap-[14px]">
            ...
            <form action="{{ route('check-booking.show') }}" method="POST">
                ...
                <button type="submit" class="w-full rounded-full p-[14px_20px] text-center font-bold text-ngekos-orange border border-ngekos-orange bg-white">
                    View My Booking
                </button>            
            </form>
        </div>
